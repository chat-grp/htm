...
exports.getTrixelCornerVectorsFromHTMName = getTrixelCornerVectorsFromHTMName;
...
function getTrixelCornerVectorsFromHTMName(name) {
    if (name.length < 2)
        throw new Error("Invalid HTM name: too short. Name must be like 'N0', 'S31', etc.");
    const baseName = name.substring(0, 2);
    const subIndicesStr = name.substring(2);
    const baseKey = baseName; // Type assertion
    ...
    let currentVertices = BASE_TRIANGLES_INDICES[baseKey];
