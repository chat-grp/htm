...
// --- Core Logic ---
function getTrixelVerticesNumeric(id: number): [Vector3DObj, Vector3DObj, Vector3DObj] {
...
function lookupIdRaDec(ra: number, dec: number, depth: number): number {
...
function unwrapLongitudeRing(ring: Array<[number, number]>): Array<[number, number]> {
    if (!ring || ring.length === 0) return [];
    const unwrappedRing: Array<[number, number]> = [[ring[0][0], ring[0][1]]];
...
class HTMmap {
  ...
  public trixelBoundary(id: number): Array<[number, number]> {
    ...
    try {
        ...
        if (id >= 1 && id <= 8) {
            ...
            if ((isNorthCap || isSouthCap) && equatorialVertices.length === 2) {
                ...
                const simplifiedRing: Array<[number, number]> = [eqV1_ll];
