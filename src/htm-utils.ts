...
type TrixelFeature = GeoJSON.Feature<GeoJSON.Polygon, TrixelProperties>;
...
export const trixelsToFC = (ids: number[]): TrixelFeatureCollection => {
  ...
  for (const id of ids) {
    try {
      ...
      const feature: GeoJSON.Feature<GeoJSON.Polygon, any> = { // Use 'any' for properties if adding extra fields
          type: 'Feature',
